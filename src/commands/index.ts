...
function statusLine(provider: string, model: string, mode: string): string {
  return `${theme.dim("provider")} ${theme.accent(provider)}  ${theme.dim("model")} ${theme.accent(model)}  ${theme.dim("mode")} ${theme.accent(mode)}`;
}

// ======= CONFIG =======
export async function cmdConfig(_args: string[]) {
  const cfg = loadConfig();
  const rl = readline.createInterface({ input: stdin, output: stdout });
  const ask = (q: string, def?: string): Promise<string> => new Promise((res) => rl.question(q, (a) => res(a.trim() || def || "")));
  ...
  if (mdl) cfg.model = mdl;

  console.log(theme.accent(`\nModes: ${MODES.join(" | ")}`));
  const mode = await ask(theme.primary(`Default mode [${cfg.mode}]: `));
  if (mode && MODES.includes(mode)) cfg.mode = mode as any;
  else if (mode) console.log(theme.warn(`Unknown mode "${mode}" ignored.`));

  const at = await ask(theme.primary(`Enable AutoTune? [${cfg.autoTune ? "yes" : "no"}]: `));
  if (at) cfg.autoTune = at.toLowerCase().startsWith("y");
...
// ======= ASK (one-shot) =======
export async function cmdAsk(args: string[]) {
...
export async function cmdChat(_args: string[]) {
  const cfg = ensureConfig();
  const provider = getProvider(cfg.provider);
  ...
  while (true) {
    ...
    let spin: ReturnType<typeof spinner> | null = spinner(`Thinking (${cfg.model})…`);
    const stopSpin = () => {
      if (spin) {
        spin.stop();
        spin = null;
    ...
    try {
      ...
      for await (const event of agentLoop(trimmed, history, {
        ...
        if (event.type === "text") {
          ...
          if (more > 0) process.stdout.write(`\n${theme.dim(`     … +${more} lines`)}`);
          process.stdout.write("\n");
...
// ======= RACE =======
export async function cmdRace(args: string[]) {
  const query = args.join(" ").trim();
  if (!query) {
...
// ======= SESSIONS =======
export async function cmdSessions(_args: string[]) {
