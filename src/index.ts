...
const ENV_HELP: Array<[string, string]> = [
...
function renderHelp(): string {
  ...
  const cmds = COMMAND_HELP.map(
    ([c, d]) => `  ${theme.accent("arb")} ${theme.primaryBold(c.padEnd(cmdWidth))} ${theme.dim(d)}`
  ).join("\n");

  const envs = ENV_HELP.map(
    ([e, d]) => `  ${theme.accent(e.padEnd(envWidth))} ${theme.dim(d)}`
  ).join("\n");
