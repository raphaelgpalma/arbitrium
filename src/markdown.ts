...
function inline(s: string): string {
  ...
  return parts
    .map((part) => {
      ...
      t = t.replace(
        /\[([^\]]+)\]\(([^)]+)\)/g,
        (_m, txt, url) => `${chalk.hex("#06b6d4").underline(txt)}${chalk.gray(` (${url})`)}`
      );
      // **bold** / __bold__
...
function renderLine(line: string, st: RenderState): string {
  ...
  if (h) {
    ...
    if (level === 1) return chalk.bold.hex("#7c3aed")(`▌ ${h[2].toUpperCase()}`);
    if (level === 2) return chalk.bold.hex("#a855f7")(`▌ ${h[2]}`);
  ...
  if (bq) return `${theme.dim("  ▏")} ${chalk.italic.gray(inline(bq[1]))}`;

  // Bullet list
  const b = line.match(/^(\s*)[-*+]\s+(.*)$/);
  if (b) return `${b[1]}${theme.primary("•")} ${inline(b[2])}`;

  // Numbered list
  const n = line.match(/^(\s*)(\d+)\.\s+(.*)$/);
  if (n) return `${n[1]}${theme.accent(`${n[2]}.`)} ${inline(n[3])}`;
